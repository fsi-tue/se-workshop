# Summary

[Einführung](./einfuehrung.md)
[Terminal / Shell](./terminal.md)

# Git

- [Git?!? Was ist das eigentlich?](./git/einfuehrung.md)
- [Installation und Konfiguration von Git](./git/installation.md)
  - [Windows](./git/installation_windows.md)
  - [MacOS](./git/installation_macos.md)
  - [Linux](./git/installation_linux.md)
- [How to git](./git/how_to_git.md)

- [Das Datenmodel](./git/datenmodel.md)
- [Datenmodell in Git](./git/datenmodell_git.md)
- [Dein erstes Repository](./git/erstes_repository.md)
- [Switch und Checkout](./git/switch_und_checkout.md)
- [Staging und Working Area](./git/staging_working_area.md)

---

- [Git im Einsatz](./git/git_im_einsatz.md)
- [Konflikte Lösen](./git/konflikte_loesen.md)
- [Tipps und Tricks](./git/tipps_und_tricks.md)

- [Remotes](./git/remotes.md)

---

- [Cheatcheat](./git/cheatcheat.md)

# Softwareentwicklung

- [Softwareentwicklung](./softwareentwicklung.md)
